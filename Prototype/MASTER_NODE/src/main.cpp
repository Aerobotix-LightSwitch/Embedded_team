...
uint8_t esp_lightnet_encryption_key[] = {0x6a,0x66,0x55,0x69,0xc3,0x97,0x46,0xf5,0xb3,0xcc,0x25,0x9d,0x32,0xb3,0xb,0xf0};
...
void esp_lightnet_init_network(){
...
Node * light_to_node(uint8_t light_number){
...
#define ESP_LIGHTNET_PING_REQ 0x01
#define ESP_LIGHTNET_PING_RES 0x02

#define ESP_LIGHTNET_NODE_UPDATE_LIGHT_STATE_REQ 0x03
#define ESP_LIGHTNET_NODE_UPDATE_LIGHT_STATE_RES 0x04


#define ESP_LIGHTNET_NODE_LIGHTS_STATUS_REQ 0x05
#define ESP_LIGHTNET_NODE_LIGHTS_STATUS_RES 0x05
...
uint8_t esp_lightnet_master_request_update(uint8_t * mac_dest,uint8_t light_number,uint8_t state){
    if(mac_dest == NULL || light_number > 127 || light_number < 1) return 1;
    
    ESP_lightnet_packet packet;
    packet.mac_source = device_mac_addr;
    packet.mac_destination = mac_dest;
    packet.identifier = ESP_LIGHTNET_NODE_UPDATE_LIGHT_STATE_REQ;
...
uint8_t esp_lightnet_master_request_status(uint8_t * mac_dest){
    if(mac_dest == NULL) return 1;
    ESP_lightnet_packet packet;
    packet.mac_source = device_mac_addr;
    packet.mac_destination = mac_dest;
    packet.identifier = ESP_LIGHTNET_NODE_LIGHTS_STATUS_REQ;
    packet.data = NULL;
    packet.data_length = 0;
    if(esp_now_send(node_list -> next_node -> mac_addr,(uint8_t *)&packet,sizeof(packet)) != ESP_OK){
        return 1;
...
uint8_t esp_lightnet_master_response_update(){
    return 0;
}

// This Function sends a light state of a node 

uint8_t esp_lightnet_master_response_light_status(uint8_t * mac_src,uint8_t * data,uint8_t data_length){
    Node * node =  mac_to_node(mac_src);
    if(mac_src == NULL || node == NULL) return 1;
...
String mac_to_str(uint8_t * mac){
    ...
    uint8_t i;
    for(i = 0; i < 6;i++){
        str_mac = String(mac[i],HEX) + ':'; 
    }
    str_mac[str_mac.length() - 1] = '\0';
    return str_mac;
}
...
void print_network_status(){
    ...
    uint16_t id = 0;
    Serial.println("\n-------------------------------------Network Status------------------------------------------");
    Serial.println("Connected Devices: " + String(network ->network_hosts));
    while(node != NULL){
...
void esp_now_onRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len){
    Node * node = mac_to_node((uint8_t *)mac_addr);
    if(node == NULL) return;
    ...
    switch(packet.identifier){
        case ESP_LIGHTNET_PING_RES:
            esp_lightnet_master_response_ping(packet.mac_source);
            break;
        case ESP_LIGHTNET_NODE_LIGHTS_STATUS_RES:
            esp_lightnet_master_lights_status(packet.mac_source,packet.data,packet.data_length);
            break;
        case ESP_LIGHTNET_NODE_UPDATE_LIGHT_STATE_RES:
            esp_lightnet_master_response_status(packet.mac_source,packet.data,packet.data_length);
            break;
        default:
            break;
    }
